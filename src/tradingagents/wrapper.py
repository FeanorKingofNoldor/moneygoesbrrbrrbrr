...
class PortfolioContextProvider:
    ...
    def __init__(self, odin_database, use_ibkr=True, ibkr_host='127.0.0.1', ibkr_port=4001):
        """
    ...
    def _get_odin_context(self):
        ...
        try:
            ...
            return {'recent_regimes': [], 'recent_candidates': [], 'data_source': 'ODIN_DB'}
    ...
    def _analyze_positions_vs_regime(self, positions, regime):
        ...
        return {
            ...
            'avg_winner_pct': sum(p['unrealized_pnl_pct'] for p in winners) / len(winners) if winners else 0,
            'avg_loser_pct': sum(p['unrealized_pnl_pct'] for p in losers) / len(losers) if losers else 0,
...
class OdinTradingAgentsWrapper:
    def __init__(self, odin_database, use_ibkr=True, ibkr_port=4002):  # Default to paper trading
        """
        Initialize ODIN TradingAgents wrapper with IBKR integration
        
        Args:
            odin_database: ODIN database instance
            use_ibkr: Whether to use IBKR for live portfolio data
        ...
        # Print setup info
        mode = "LIVE" if ibkr_port == 4001 else "PAPER"
        ibkr_status = "ENABLED" if use_ibkr else "DISABLED"
        print(f"ODIN Setup: IBKR {ibkr_status}, Mode: {mode} (port {ibkr_port})")
    ...
    def analyze_stock(self, symbol, date=None):
        ...
        print(f"Market Regime: {regime_context['regime']} (F&G: {regime_context['fear_greed_value']})")
        print(f"Portfolio: {portfolio_context['total_positions']} positions, "
...
- Current Regime: {regime_context['regime']} ({regime_context['fear_greed_text']})
...
"""
        ...
        # Pass the context to TradingAgents
    ...
    def get_portfolio_summary(self):
        ...
        print(f"\n{'='*60}")
        print(f"ODIN PORTFOLIO SUMMARY - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"{'='*60}")
    ...
    def analyze_with_context_injection(self, symbol, date=None):
        """
        Alternative method that tries to inject context into TradingAgents
        This would require modifying TradingAgents to accept initial context
        """
        if date is None:
            date = "2024-12-19"
        ...
        # Create initial state with context (conceptual - needs TradingAgents support)
        initial_state = {
            'company_of_interest': symbol,
            'trade_date': date,
            'market_regime': regime_context,      # NEW
            'portfolio_context': portfolio_context,  # NEW
            'messages': []
        }
        
        # This would require TradingAgents modification to accept initial_state
        # result, decision = self.graph.propagate_with_state(initial_state)
        
        # For now, fall back to regular analysis
        return self.analyze_stock(symbol, date)