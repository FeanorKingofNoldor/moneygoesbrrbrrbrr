...
class OdinTradingAgentsWrapper:
    def __init__(
        self, odin_database, use_ibkr=True, ibkr_port=4002
        ...
        self.config["backend_url"] = (
            "https://api.openai.com/v1"  # OpenAI's actual endpoint
        )
        self.config["deep_think_llm"] = "gpt-4o-mini"  # Cheap and fast
        self.config["quick_think_llm"] = "gpt-4o-mini"  # $0.15 per 1M input tokens
        self.config["online_tools"] = True
        self.config["max_debate_rounds"] = 1  # Reduce costs

        # Initialize ODIN components
        self.regime_detector = OdinRegimeDetector()
        self.portfolio_provider = PortfolioContextProvider(
            odin_database, use_ibkr=use_ibkr, ibkr_port=ibkr_port
        )
    ...
    """
        ...
        result, decision = self.graph.propagate(symbol, date, regime_context)
